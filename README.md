...
**Size it to your callers.** A cache that mostly misses is overhead; a cache that fits the working set is worth most where there are many distinct certificates, because the JVM's own parsed-certificate cache (`X509Factory`, 750 soft-referenced entries) stops covering there: with 2000 distinct certificates, resolving one costs 12.4 us uncached against 2.0 us cached. Turn the cache off (`org.cloudfoundry.router.certificate.cache.enabled=false`) if you cannot size it to the population, or if the memory does not suit your deployment. `org.cloudfoundry.router.certificate.cache.size` sets the entries per generation (two generations, so `2 x size` entries); invalid or non-positive values are ignored and the default is used, with a warning logged. Each entry retains the header string plus the parsed certificate, so the rough bound is `2 x size x (header bytes + parsed certificate)` -- about 0.4 MB at the default for CF-sized headers while the JVM's cert cache also holds them, and up to ~2 MB when it does not, proportionally more where `maxHttpHeaderSize` has been raised for large certificates. The filter logs its effective cache configuration at `INFO` on startup and warns when the hit rate is low.
